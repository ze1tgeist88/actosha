    ...
    def generate_signals(self, df):
        ...
        for i, idx in enumerate(data.index):
            if not po:
                ...
                dev = (data.loc[idx, "close"] - data.loc[idx, "vwap"]) / data.loc[idx, "vwap"]
                bh = i - ei
                if ps == "long" and (dev >= -self._et or bh >= self._mpd):
                    sigs.at[idx, "side"] = "close"; sigs.at[idx, "strength"] = 0.5; po = False; ps = None
                elif ps == "short" and (dev <= self._et or bh >= self._mpd):
                    sigs.at[idx, "side"] = "close"; sigs.at[idx, "strength"] = 0.5; po = False; ps = None
        return SignalBundle(signals=sigs, metadata={"strategy": self.name, "params": self.get_params()})
...
def run_one(factory, params, df, ic, comm, slip):
    ...
    try:
        r = e.run(s, df, initial_capital=ic, commission=comm, slippage=slip)
        return r
    except Exception:
        return None
...
def fm(r):
    ...
    m = r.metrics
    return {
        "ret": round((m.get("total_return", 0) or 0) * 100, 2),
        "shr": round(m.get("sharpe_ratio") or 0, 2),
        "dd": round(m.get("max_drawdown_pct") or 0, 2),
...
def build_md(table):
    lines = []
    lines.append("# ACTosha - Binance Futures Backtest 2025-04-25 to 2026-04-26\n\n")
    lines.append("| Metric | Value |\n")
    lines.append("|--------|-------|\n")
    lines.append("| Period | 2025-04-25 to 2026-04-26 |\n")
    ...
    for sym in ["BTC", "ETH"]:
        for tf in ["1h", "4h", "1d"]:
            ...
            for row_label, row_val in [
                ...
                lines.append(f"{row_label} | {row_val}\n")
            lines.append("\n")
...
def main():
    ...
    print("ACTosha - Binance Futures Backtest 2025-04-25 to 2026-04-26")
    print("=" * 60)
    ...
    for sym in syms:
        for tf in tfs:
            ...
            for name in ORDER:
                ...
                m = fm(best["result"])
                if not m:
                    print(f"  {name}: ERROR ({elapsed:.1f}s)")
                    table.append({"sym": sym_label, "tf": tf, "strat": name,
                ...
                table.append({"sym": sym_label, "tf": tf, "strat": name,
                              ...
                              "params": best["params"], "error": False})

    print("\n[3/3] Writing report...")
    md = build_md(table)
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backtest_binance_final.md")
    with open(out_path, "w") as f:
        f.write(md)
    print(f"  -> {out_path}")
    print(f"\nTotal rows: {len(table)}")
    errors = [r for r in table if r.get("error")]
